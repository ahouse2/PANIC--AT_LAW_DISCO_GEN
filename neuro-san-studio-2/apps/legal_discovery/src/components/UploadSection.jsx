...
function UploadSection() {
  ...
  const graphRef = useRef({ start: Date.now(), points: [], lastTick: Date.now(), lastDone: 0 });
  const [rate, setRate] = useState(0); // files/sec instantaneous
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const dt = Math.max(1, (now - (graphRef.current.lastTick || now)) / 1000);
      const dDone = Math.max(0, done - (graphRef.current.lastDone || 0));
      const filesPerSec = dDone / dt;
      graphRef.current.lastTick = now;
      graphRef.current.lastDone = done;
      setRate(filesPerSec);
      // Normalize to percentage-like scale for the sparkline: clamp to 100 f/s
      const v = Math.min(100, Math.round(filesPerSec * 100) / 1); // 1 f/s => 100
      ...
      {jobs.length>0 && (
        <div className="mb-3" role="status" aria-live="polite">
          <div className="mb-2 flex items-center justify-between">
            <div className="text-xs text-cyan-300/90">Throughput</div>
            <div className="text-xs text-cyan-200 font-mono">{rate.toFixed(2)} files/s</div>
          </div>
          <UploadGraph data={trend} width={560} height={72} />
          <VirtualList
            items={jobs}
            itemHeight={28}
            height={196}
