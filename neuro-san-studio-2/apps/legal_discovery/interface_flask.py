...
# --- Vector convenience endpoints (passthrough to MCP wrappers) ------------
@app.route("/api/vector/count", methods=["GET"])
def vector_count():
    t0 = time.perf_counter()
    try:
        from panic_core.mcp import mcp_vector_database

        case_id = request.args.get("case_id")
        res = mcp_vector_database.count(case_id=case_id)
        ...
        resp = make_response(jsonify({"ok": True, "result": res, "ms": elapsed_ms}))
        ...
        resp = make_response(jsonify({"ok": False, "error": str(exc), "ms": elapsed_ms}))
...
def vector_search_legacy():
    ...
    try:
        ...
        n = int(request.args.get("n_results") or 10)
        case_id = request.args.get("case_id")
        where_raw = request.args.get("where")
        where = None
        if where_raw:
            try:
                where = _json.loads(where_raw)
            except Exception:
                where = None
        res = mcp_vector_database.query(q=q, n_results=n, where=where, case_id=case_id)
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info(
            "vector_search",
            ok=True,
...
# --- Drafting endpoints (AutoDrafter via MCP) ------------------------------
@app.route("/api/drafting/available", methods=["GET"])
def drafting_available():
    t0 = time.perf_counter()
    try:
        from panic_core.mcp import mcp_drafting

        res = mcp_drafting.available()
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_available", ok=True, ms=elapsed_ms, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": True, "result": res, "ms": elapsed_ms}))
        resp.status_code = 200
        return resp
    except Exception as exc:  # pragma: no cover
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_available", ok=False, error=str(exc), ms=elapsed_ms, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": False, "error": str(exc), "ms": elapsed_ms}))
        resp.status_code = 500
        return resp


@app.route("/api/drafting/generate", methods=["POST"])
def drafting_generate():
    import json as _json
    t0 = time.perf_counter()
    try:
        from panic_core.mcp import mcp_drafting
        ...
        if not isinstance(payload, dict):
            payload = _json.loads(request.data.decode("utf-8") or "{}")
        motion_type = (payload.get("motion_type") or "").strip()
        temperature = payload.get("temperature")
        out = mcp_drafting.generate(motion_type=motion_type, temperature=temperature)
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_generate", ok=True, ms=elapsed_ms, motion_type=motion_type, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": True, "result": out, "ms": elapsed_ms}))
        resp.status_code = 200
        return resp
    except Exception as exc:  # pragma: no cover
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_generate", ok=False, error=str(exc), ms=elapsed_ms, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": False, "error": str(exc), "ms": elapsed_ms}))
        resp.status_code = 500
        return resp


@app.route("/api/drafting/export", methods=["POST"])
def drafting_export():
    import json as _json
    t0 = time.perf_counter()
    try:
        ...
        fmt = payload.get("fmt")
        out = mcp_drafting.export(content=content, file_path=path, fmt=fmt)
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_export", ok=True, ms=elapsed_ms, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": True, "result": out, "ms": elapsed_ms}))
        resp.status_code = 200
        return resp
    except Exception as exc:  # pragma: no cover
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        logger.info("drafting_export", ok=False, error=str(exc), ms=elapsed_ms, request_id=getattr(g, "request_id", None))
        resp = make_response(jsonify({"ok": False, "error": str(exc), "ms": elapsed_ms}))
        resp.status_code = 500
        return resp

# --- Direct tool runner (allowlisted) ----------------------------------------
@app.route("/api/agents/run_tool_direct", methods=["GET", "POST"])
def run_tool_direct():
    try:
        import json as _json
        ...
        allowlist = {
            "mcp_vector.count": lambda **k: mcp_vector_database.count(),
            ...
            # Drafting (basic allowlist)
            "mcp_drafting.available": lambda **k: __import__('panic_core.mcp.mcp_drafting', fromlist=['x']).available(),
            "mcp_drafting.generate": lambda **k: __import__('panic_core.mcp.mcp_drafting', fromlist=['x']).generate(**k),
            "mcp_drafting.export": lambda **k: __import__('panic_core.mcp.mcp_drafting', fromlist=['x']).export(**k),
        }
        payload = request.get_json(silent=True)
        if not isinstance(payload, dict):
            try:
                ...
                payload = {}
        tool = (payload.get("tool") if isinstance(payload, dict) else None) or request.args.get("tool")
        args = (payload.get("args") if isinstance(payload, dict) else None) or {}
        if not tool or tool not in allowlist:
            resp = make_response(jsonify({"ok": False, "error": "tool not allowed or missing"}))
        ...
        resp = make_response(jsonify({"ok": False, "error": str(exc)}))
        resp.status_code = 500
        return resp


@app.route("/api/ping_direct", methods=["GET"])
def ping_direct():
    return "ok"
...
def orchestrate_action():
    """Run a minimal orchestrated flow using MCP tools.

    Body:
      { "action": "extract|vector_count|extract_and_vector", "filepath": "/path", "case_id": 1 }
    """
    import json as _json
    try:
        payload = request.get_json(silent=True)
    ...
    case_id = int(payload.get("case_id") or 1)
    from panic_core.mcp import (
        mcp_document_processor,
        mcp_vector_database,
        mcp_knowledge_graph,
    ...
    t0 = time.perf_counter()
    try:
        if action == "vector_count":
            res = mcp_vector_database.count()
            elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
            logger.info(
                "orchestrate",
                action=action,
            ...
            resp = make_response(jsonify({"ok": True, "action": action, "result": res, "ms": elapsed_ms}))
            resp.status_code = 200
            return resp
        elif action == "extract":
            if not filepath:
                ...
                resp = make_response(jsonify({"ok": False, "error": "filepath required", "ms": elapsed_ms}))
                resp.status_code = 400
                return resp
            res = mcp_document_processor.extract(filepath)
            elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
            logger.info(
                "orchestrate",
                action=action,
            ...
            payload_out = {
                "ok": True,
                "action": action,
                "result": {"bytes": len(res.get("text", "")), "meta": res.get("meta", {})},
                "ms": elapsed_ms,
            }
            resp = make_response(jsonify(payload_out))
            resp.status_code = 200
            return resp
        elif action == "extract_and_vector":
            if not filepath:
                elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
                logger.info(
                    "orchestrate",
                ...
                resp.status_code = 400
                return resp
            doc = mcp_document_processor.extract(filepath)
            text = doc.get("text") or ""
            meta = doc.get("meta") or {}
            ids = [f"{os.path.basename(filepath)}:0"]
            mcp_vector_database.add_documents(
                [text], [meta | {"case_id": case_id, "path": filepath}], ids, case_id=case_id
            )
            cnt = mcp_vector_database.count()
            elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
            logger.info(
                "orchestrate",
                action=action,
                ok=True,
                ms=elapsed_ms,
                vector_count=cnt.get("count", 0),
                request_id=getattr(g, "request_id", None),
            )
            payload_out = {
                "ok": True,
                "action": action,
                "result": {"added": 1, "vector_count": cnt.get("count", 0)},
                "ms": elapsed_ms,
            }
            resp = make_response(jsonify(payload_out))
            resp.status_code = 200
            ...
            resp = make_response(jsonify({"ok": False, "error": "unknown action", "ms": elapsed_ms}))
            resp.status_code = 400
            return resp
    except Exception as exc:
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
...
def internal_error_handler(exc):  # pragma: no cover
    try:
        import traceback
        print("INTERNAL_ERROR:", repr(exc))
        traceback.print_exc()
    except Exception:
        pass
    payload = {"ok": False, "error": {"code": "internal_error", "message": "unexpected error"}}
...
# Remove duplicate handler; single handler defined above
...
MAX_TIMEOUT = int(os.environ.get("FILE_HASH_TIMEOUT", "120"))  # seconds per file (extended)
...
@app.route("/api/vector/count_legacy", methods=["GET"])
def vector_count_legacy():
